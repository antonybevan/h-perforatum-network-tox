...
& Hyperforin & 10 & $d_c = 1.30$ & $-$3.86 & $<0.001$$^\dagger$ & --- \\
& Quercetin & 62 & $d_c = 1.68$ & \textbf{$-$5.44} & $<0.001$$^\dagger$ & --- \\
...
& Hyperforin & 10 & 0.1138 & \textbf{+10.27} & $<0.001$$^\dagger$ & 0.1138 \\
& Quercetin & 62 & 0.0322 & +4.42 & $<0.001$ & 0.0322 \\
...
& Hyperforin & 10 & 0.1330 & \textbf{+9.07} & $<0.001$$^\dagger$ & 0.1330 \\
& Quercetin & 62 & 0.0493 & +5.56 & $<0.001$ & 0.0493 \\
\bottomrule
\end{tabular}
...
\footnotesize
$^\dagger$At permutation floor ($<$1/1,000).\\
PTNI = per-target network influence; $d_c$ = mean minimum shortest-path distance.
\end{minipage}
\end{table}
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Analysis} & \textbf{Hyperforin PTNI} & \textbf{Quercetin PTNI} & \textbf{Efficiency Ratio} & \textbf{Robust Ratio}$^*$ \\
\midrule
RWR (topology-only) & 0.1138 & 0.0322 & \textbf{3.5}$\times$ & \textbf{3.7}$\times$ \\
EWI (expression-weighted) & 0.1330 & 0.0493 & \textbf{2.7}$\times$ & \textbf{2.7}$\times$ \\
\bottomrule
\end{tabular}
...
\begin{minipage}{\linewidth}
\footnotesize
$^*$Robust Ratio is defined as the Observed Influence divided by the size-matched Bootstrap Mean (N=10 targets).
\end{minipage}
\end{table}
...
\midrule
$\geq$700 & Hyperforin & \textbf{+10.90} & +10.46 & 0.60 & $-$6.04 \\
(11,693 nodes) & Quercetin & +5.16 & +6.65 & 1.34 & $-$5.46 \\
\addlinespace
$\geq$900 & Hyperforin & \textbf{+10.27} & +9.07 & 1.30 & $-$3.86 \\
(7,677 nodes) & Quercetin & +4.42 & +5.56 & 1.68 & $-$5.44 \\
\bottomrule
\end{tabular}
...
\begin{minipage}{\linewidth}
\footnotesize
Note: At $\geq$900, Quercetin achieves a more "significant" proximity Z-score despite being physically more distant ($1.68$ vs $1.30$) from DILI genes. In contrast, influence-based rankings (RWR and EWI) remain stable across multiple STRING confidence tiers, correctly prioritizing the regulatory hub modulator (Hyperforin) in all configurations.
\end{minipage}
\end{table}
