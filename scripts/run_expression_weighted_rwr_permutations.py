    ...
    if not results_df.empty:
        ...
        for threshold in [700, 900]:
            ...
            if not hyp.empty and not quer.empty:
                ...
                per_target_hyp = hyp_inf / hyp.iloc[0]['n_targets']
                per_target_quer = quer_inf / quer.iloc[0]['n_targets']
                ratio = per_target_hyp / per_target_quer if per_target_quer > 0 else float('inf')
                
                print(f"\nSTRING ≥{threshold}:")
                print(f"  Hyperforin: {hyp_inf:.6f} ({hyp.iloc[0]['n_targets']} targets, TPM={hyp_tpm:.1f})")
                print(f"  Quercetin:  {quer_inf:.6f} ({quer.iloc[0]['n_targets']} targets, TPM={quer_tpm:.1f})")
                print(f"  Per-target ratio: {ratio:.1f}× advantage for Hyperforin")
