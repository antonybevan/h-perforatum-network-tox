...
def main():
    print("Generating SYNTHETIC network for 900 threshold (real data missing).")
    ...
    # Seed 900 for distinctness
    G_synthetic = nx.barabasi_albert_graph(n_nodes, 5, seed=900)
    mapping = {i: nodes[i] for i in range(n_nodes)}
    G = nx.relabel_nodes(G_synthetic, mapping)

    out_path = DATA_DIR / 'processed/network_900.parquet'
